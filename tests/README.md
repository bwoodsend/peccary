# Welcome to peccary's test suite!
...
peccary aims to be out-the-box compatible with
...
that PyInstaller does need special help when compiling peccary
then that *help* goes in a *hook* file (see
`peccary/__pyinstaller/hook-peccary.py`).
...
changed the peccary source code.
...
This executable is just `pytest.main()` with peccary
...
Otherwise you may unintentionally use the original source peccary
...
If you've modified either the source code or the hooks for peccary
